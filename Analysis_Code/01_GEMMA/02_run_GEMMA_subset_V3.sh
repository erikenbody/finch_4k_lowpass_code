...
##easy download of assoc files:
#cd to data/GEMMA
##rsync -zarv --prune-empty-dirs --include="*/" --include="*assoc.txt" --exclude="*" upp:/crex/proj/snic2020-2-19/private/darwins_finches/users/erikenbody/Finch_lowpass/lowpass/GEMMA/ .
...
cd $WORK_D
#now running this as genome-wide, not intervals, so call everything autosomes
CHR=autosomes
...
bcftools annotate --set-id '%CHROM\_%POS\_%REF\_%FIRST_ALT' -O z -o ${CHR}_merge_ID.vcf.gz $VCF
...
qctool -g ${CHR}_merge_ID.vcf.gz -ofiletype bimbam_dosage -og ${OUTNAME}/${CHR}_${OUTNAME}.geno -incl-samples $SUBSET
########################################################################################################################
